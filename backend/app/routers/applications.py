...
def apply_opportunity(
    ...
    send_notification(
        ...
        link_url=f"/industry/applications"
    )
...
def get_applications(
    ...
    if current_user.role in ["student", "faculty"]:
        ...
        opp_ids = [o.id for o in db.query(Opportunity.id).filter(Opportunity.posted_by_user_id == current_user.id).all()]
...
def update_application_status(
    ...
    if data.status == "selected" and opp.type in ["internship", "faculty_internship", "industrial_training", "live_project"]:

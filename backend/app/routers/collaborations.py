...
def get_collaborations(
    ...
    if current_user.role == "institution" and current_user.institution_id:
        ...
        query = query.filter(
            (Collaboration.initiator_user_id == current_user.id) | (Collaboration.partner_industry_id == ind_id)
        )
...
def create_collaboration_proposal(
    ...
    if collab.partner_institution_id:
        inst_users = (
            db.query(User)
            .filter(User.institution_id == collab.partner_institution_id, User.role == "institution")
            .all()
        )
        for u in inst_users:
            send_notification(

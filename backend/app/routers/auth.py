...
def register(data: RegisterRequest, request: Request, db: Session = Depends(get_db)):
    # 1. Validate role
    allowed_roles = ["student", "faculty", "industry", "institution"]
    if data.role not in allowed_roles:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid registration role. Allowed: {allowed_roles}"
        )
...
def login(data: LoginRequest, request: Request, db: Session = Depends(get_db)):
    user = db.query(User).filter(
        (User.email == data.username_or_email) | (User.username == data.username_or_email)
    ).first()
...
def get_me(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if current_user.role == "student" and current_user.student_profile:
        profile_data = {
            ...
            "institution_name": current_user.student_profile.institution.name if current_user.student_profile.institution else None
        }
        ...
        profile_data = {
            ...
            "institution_name": current_user.faculty_profile.institution.name if current_user.faculty_profile.institution else None
        }

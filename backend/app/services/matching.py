...
from app.models.skill import StudentSkill, CareerRole, CareerRoleSkill, Skill
...
def evaluate_student_opportunity_match(
    ...
    if opportunity.eligibility_cgpa and opportunity.eligibility_cgpa > 0:
        if not student.cgpa or student.cgpa < opportunity.eligibility_cgpa:
            is_eligible = False
            reasons.append(f"Eligibility note: Opportunity requires minimum CGPA of {opportunity.eligibility_cgpa}. Student CGPA is {student.cgpa or 'N/A'}.")
        else:
            reasons.append(f"Meets CGPA requirement (Student: {student.cgpa} >= Required: {opportunity.eligibility_cgpa}).")
    ...
    if opportunity.eligibility_year and student.year_of_study:
        if student.year_of_study != opportunity.eligibility_year:
            ...
            reasons.append(f"Year of study note: Targeted for Year {opportunity.eligibility_year} (Student is in Year {student.year_of_study}).")
        else:
    ...
    for req in opp_skills:
        ...
        if req.skill_id in student_skill_map:
            ...
            if stu_level_val >= req_level_val:
                ...
                reasons.append(f"Strong skill match: {skill_name} at {ss.skill_level.capitalize()} level{verified_tag}.")
            else:
                # Partial credit
                ratio = stu_level_val / float(req_level_val)
                earned_skill_weight += weight * ratio
                reasons.append(f"Developing skill match: {skill_name} is at {ss.skill_level.capitalize()} level (Target: {req.minimum_level.capitalize()}).")
        else:

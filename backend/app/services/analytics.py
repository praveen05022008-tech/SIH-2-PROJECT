...
def get_institution_analytics(db: Session, institution_id: int) -> Dict[str, Any]:
    ...
    student_ids = [
        s.id for s in db.query(StudentProfile.id).filter(StudentProfile.institution_id == institution_id).all()
    ]
    user_ids = [
        s.user_id
        for s in db.query(StudentProfile.user_id).filter(StudentProfile.institution_id == institution_id).all()
    ]
    ...
    if user_ids:
        ...
        students_placed = (
            ...
            .filter(Application.applicant_user_id.in_(user_ids), Application.status == "selected")
            .count()
        )
...
def get_industry_analytics(db: Session, user_id: int) -> Dict[str, Any]:
    ...
    breakdown = [
        {"id": o.id, "title": o.title, "type": o.type, "status": o.status, "openings": o.openings_count}
